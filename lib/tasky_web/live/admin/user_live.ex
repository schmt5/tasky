defmodule TaskyWeb.Admin.UserLive do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="page-header">
        <div class="max-w-5xl mx-auto">
          ...
          <h1>
            Benutzer <em>verwalten</em>
          </h1>
          <p>Übersicht und Bearbeitung aller Benutzer auf der Plattform.</p>
        </div>
      </div>
      ...
      <div class="max-w-5xl mx-auto mt-10 bg-white border border-stone-200 rounded-[14px] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.07),0_1px_2px_rgba(0,0,0,0.04)]">
        ...
        <%= if @users == [] do %>
          <div class="flex flex-col items-center text-center py-14 px-8">
            ...
            <p class="text-[13px] text-stone-400 max-w-[280px] leading-relaxed">
              <%= if filters_active?(@search, @role_filter, @class_filter) do %>
        ...
        <% else %>
          <.table
            id="users"
            rows={@users}
            row_click={fn user -> JS.navigate(~p"/admin/users/#{user.id}/edit") end}
          >
  ...
  def handle_event("filter", params, socket) do
    {:noreply,
     ...
     |> assign(:role_filter, Map.get(params, "role", socket.assigns.role_filter))
  ...
  defp filters_active?(search, role, class) do
    String.trim(search) != "" or role != "" or class != ""
  end
end
